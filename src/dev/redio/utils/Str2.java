...
public interface Str2 
    ...
    public static Str2 of(CharSequence cs) {
        return new StrImpl(cs);
    }

    @Override
    Str2 subSequence(int start, int end);

    default Str2 subSequence(int start) {
    ...
    default Str2 concat(CharSequence other) {
        return Str2.of(new LinkedSequence(this, other));
    }
    ...
    default Stream<Str2> lines() {
        final class Splitter implements Spliterator<Str2> {
            ...
            private Str2 next() {
                ...
                return Str2.this.subSequence(lastIndex, endOfLine);
            }

            private int endOfLine(int start) {
                for (int i = start; i < this.length; i++) {
                    char c = Str2.this.charAt(i);
                    if (c == '\n' || c == '\r')
                        return i;
                }
                return this.length;
            ...
            private int startOfLine(int endOfLine) {
                endOfLine++;
                if (endOfLine < this.length && Str2.this.charAt(endOfLine) =='\n')
                    endOfLine++;
                return endOfLine;
            }

            @Override
            public boolean tryAdvance(Consumer<? super Str2> action) {
                if (index >= length)
                    return false;
            ...
            @Override
            public Spliterator<Str2> trySplit() {
